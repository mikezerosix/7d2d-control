...
##  ~/7d2d.pid : pid file (Process id) that works as lock file. Start up creates this and stop deletes this. This pevents starting the server process multiple times. Server crash leaves this file which prvents starting, delete the file to start IF you are sure process is not already running.
##
## ------------------------
...
case "$1" in
  start)
    ...
    echo "Starting 7dtd..."
    ${INSTALL_DIR}/7DaysToDieServer.x86${bitCount} -logfile ${HOME_DIR}/7d2d.log -quit -batchmode -nographics -configfile=${INSTALL_DIR}/serverconfig.xml -dedicated &
  ...
  stop)
    ...
    exec 3<>/dev/tcp/localhost/${TELNET_PORT}
